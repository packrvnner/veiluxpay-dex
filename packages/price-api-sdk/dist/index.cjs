...
var CHAINS_FOR_NEW_WALLET_API = [chains.NonEVMChainId.SOLANA];
var NATIVE_ADDRESSES = {
  [chains.NonEVMChainId.SOLANA]: "So11111111111111111111111111111111111111112"
};
...
function parseQuoteResponse(res, {
  ...
  if (bestOrder.type === "PCS_CLASSIC" /* PCS_CLASSIC */) {
    return {
      ...
      trade: smartRouter.InfinityRouter.Transformer.parseTrade(chainId, bestOrder.order)
    };
  }
  if (bestOrder.type === "DUTCH_LIMIT" /* DUTCH_LIMIT */) {
    ...
    return {
      ...
      ammTrade: otherAmmTrade ? smartRouter.InfinityRouter.Transformer.parseTrade(chainId, otherAmmTrade.order) : void 0,
...
function parseAMMPriceResponse(chainId, res) {
  ...
  const trade = smartRouter.InfinityRouter.Transformer.parseTrade(chainId, rest);
  return {
    ...trade,
