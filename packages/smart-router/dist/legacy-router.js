...
chunkH57NM25L_js.__export(legacyRouter_exports, {
...
async function getPairs(currencyPairs, { provider, chainId }) {
  const tokens = currencyPairs.map(([currencyA, currencyB]) => [
    chunkH57NM25L_js.wrappedCurrency(currencyA, chainId),
    chunkH57NM25L_js.wrappedCurrency(currencyB, chainId)
  ]);
...
async function getStableSwapOutputAmount(pair, inputAmount, { provider }) {
  const wrappedInputAmount = chunkH57NM25L_js.wrappedCurrencyAmount(inputAmount, inputAmount.currency.chainId);
...
function getStableSwapFeeCall(pair, inputAmount) {
  ...
  const wrappedInputAmount = chunkH57NM25L_js.wrappedCurrencyAmount(inputAmount, chainId);
  if (!wrappedInputAmount) {
    throw new Error(`No wrapped token amount found for input amount: ${inputAmount.currency.name}`);
  }
  ...
  return {
    ...
    address: pair.infoStableSwapAddress || chunkH57NM25L_js.STABLE_SWAP_INFO_ADDRESS[chainId],
...
async function getAllCommonPairs(currencyA, currencyB, { provider }) {
  ...
  const additionalA = tokenA ? chunkH57NM25L_js.ADDITIONAL_BASES[chainId]?.[tokenA.address] ?? [] : [];
  const additionalB = tokenB ? chunkH57NM25L_js.ADDITIONAL_BASES[chainId]?.[tokenB.address] ?? [] : [];
...
function createGetBestTradeFromV2(tradeType) {
  ...
  return async function bestTradeFromV2(amountIn, output, options) {
    ...
    for (let i = 1; i <= maxHops; i++) {
      ...
      if (sdk.isTradeBetter(bestTradeSoFar, currentTrade, chunkH57NM25L_js.BETTER_TRADE_LESS_HOPS_THRESHOLD)) {
