...
var chunkH57NM25L_js = require('./chunk-H57NM25L.js');
...
chunkH57NM25L_js.__export(infinity_router_exports, {
...
function getTokenPrice(pool, base, quote) {
  ...
  if (isInfinityClPool(pool)) {
    ...
    const v3Pool = new v3Sdk.Pool(currency0.asToken, currency1.asToken, fee, sqrtRatioX96, liquidity, tick);
...
function convertInfinityStablePoolToStablePool(pool) {
  ...
  const balance0 = pool.reserve0 ?? sdk.CurrencyAmount.fromRawAmount(pool.currency0, 0);
  const balance1 = pool.reserve1 ?? sdk.CurrencyAmount.fromRawAmount(pool.currency1, 0);
  return {
    type: 2 /* STABLE */,
    address: pool.hooks,
    balances: [balance0, balance1],
    amplifier: BigInt(pool.amplifier ?? 0),
    fee: new sdk.Percent(BigInt(pool.stableFee ?? 0), infinityStableSdk.INFINITY_STABLE_POOL_FEE_DENOMINATOR)
  };
}
...
function encodeInfinityRouteToPath(route, exactOutput) {
  ...
  const { path } = pools.reduce(
    ({ baseCurrency, path: path2 }, pool) => {
      ...
      const hooksRegistration = pool.hooksRegistrationBitmap !== void 0 ? infinitySdk.decodeHooksRegistration(pool.hooksRegistrationBitmap) : void 0;
      const parameters = infinitySdk.encodePoolParameters(
      ...
      return {
        ...
        path: [
          ...path2,
          {
            ...
            fee: infinitySdk.isDynamicFeeHook(pool.currency0.chainId, pool.hooks) ? infinitySdk.DYNAMIC_FEE_FLAG : pool.fee,
...
function encodeMixedRouteToPath(route, exactOutput) {
  ...
  const { path, types } = route.pools.reduce(
    ({ inputToken, path: path2, types: types2 }, pool, index) => {
      ...
      const fee = isV3Pool(pool) ? pool.fee : chunkH57NM25L_js.V2_FEE_PATH_PLACEHOLDER;
      if (index === 0) {
        return {
...
function getUsdGasToken(chainId) {
  return chunkH57NM25L_js.usdGasTokensByChain[chainId]?.[0] ?? null;
}
...
var _SwapRouter = class {
  ...
  static encodeV2Swap(trade, options, routerMustCustody, performAggregatedSlippageCheck) {
    ...
    const recipient = routerMustCustody ? chunkH57NM25L_js.ADDRESS_THIS : typeof options.recipient === "undefined" ? chunkH57NM25L_js.MSG_SENDER : sdk.validateAndParseAddress(options.recipient);
    if (trade.tradeType === sdk.TradeType.EXACT_INPUT) {
    ...
    for (const route of trade.routes) {
      ...
      if (singleHop) {
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          const recipientAddress = lastSectionInRoute ? recipient : chunkH57NM25L_js.ADDRESS_THIS;
...
chunkH57NM25L_js.__export(transformer_exports, {
    ...
    try {
      ...
      Object.keys(data).forEach((key) => {
        ...
        for (const [address, list] of Object.entries(data[key])) {
          const src = viem.checksumAddress(address);
          const to = list.map((item) => {
            return new swapSdkCore.Token(chainId, viem.checksumAddress(item.address), item.decimals, item.symbol, "");
          });
      ...
      return chunkH57NM25L_js.ADDITIONAL_BASES;
    }
  },
  () => {
...
var getCheckAgainstBaseTokens = memoize(
  async (currencyA, currencyB) => {
    ...
    const [tokenA, tokenB] = chainId ? [chunkH57NM25L_js.wrappedCurrency(currencyA, chainId), chunkH57NM25L_js.wrappedCurrency(currencyB, chainId)] : [void 0, void 0];
    if (!tokenA || !tokenB) {
      return [];
    }
    const common = chunkH57NM25L_js.BASES_TO_CHECK_TRADES_AGAINST[chainId] ?? [];
...
var getPairCombinations = memoize(
  async (currencyA, currencyB) => {
    ...
    return [
      ...
      if (!chainId)
        return true;
      const customBases = chunkH57NM25L_js.CUSTOM_BASES[chainId];
...
var poolInfoStr = (pool) => {
  switch (pool.type) {
    ...
    case 0 /* V2 */: {
      const id = sdk.Pair.getAddress(pool.reserve0.currency.wrapped, pool.reserve1.currency.wrapped);
...
function getBestSwapRouteBy(tradeType, percentToQuotes, percents, chainId, by, { maxSplits = 4, minSplits = 0 }, quoteId) {
  ...
  if (!chunkH57NM25L_js.usdGasTokensByChain[chainId] || !chunkH57NM25L_js.usdGasTokensByChain[chainId][0]) {
    throw new Error(`Could not find a USD token for computing gas costs on ${chainId}`);
  }
  const usdToken = chunkH57NM25L_js.usdGasTokensByChain[chainId][0];
...
function withCache(fetchPrices, options) {
  ...
  return async ({ addresses, chainId }) => {
    ...
    if (requireLlamaChain && !chains.getLlamaChainName(chainId)) {
    ...
    for (const result of freshResults) {
      cache.set(viem.getAddress(result.address), result);
    }
...
var createGetTokenPriceFromLlma = ({ endpoint }) => {
  return async ({ addresses, chainId }) => {
    const list = addresses.map((address) => `${chains.getLlamaChainName(chainId)}:${address.toLocaleLowerCase()}`).join(",");
...
var getV2PoolsOnChain = createOnChainPoolFactory({
  abi: chunkH57NM25L_js.pancakePairABI,
...
var getStablePoolsOnChain = createOnChainPoolFactory({
  abi: chunkH57NM25L_js.stableSwapPairABI,
  getPossiblePoolMetas: async ([currencyA, currencyB]) => {
...
function poolSelectorFactory({
  ...
  return function tvlSelector(currencyA, currencyB, unorderedPoolsWithTvl) {
    ...
    const baseTokens = chunkH57NM25L_js.BASES_TO_CHECK_TRADES_AGAINST[chainId] ?? [];
...
var createFallbackTvlRefGetter = () => {
  ...
  return async (params) => {
    ...
    if (chains.isTestnetChainId(currencyA?.chainId)) {
      return [];
    }
...
function encodeInfinityMixedRouteCurrencyPath(route) {
  ...
  const pathStart = swapSdkCore.getMatchedCurrency(currencyStart, getPoolCurrencies(pools[0]));
  if (!pathStart) {
  ...
  const { path } = pools.reduce(
    ({ baseCurrency, path: path2 }, p) => {
      ...
      return {
        baseCurrency: quoteCurrency,
        path: [...path2, swapSdkCore.getCurrencyAddress(quoteCurrency)]
      };
    },
    { baseCurrency: pathStart, path: [swapSdkCore.getCurrencyAddress(pathStart)] }
  );
  return path;
}
  ...
  return route.pools.map((p) => {
    ...
    if (isInfinityClPool(p) || isInfinityStablePool(p)) {
      const poolKey = {
        ...
        fee: infinitySdk.isDynamicFeeHook(p.currency0.chainId, p.hooks) ? infinitySdk.DYNAMIC_FEE_FLAG : p.fee,
        hooks: p.hooks,
        poolManager: p.poolManager,
        parameters: {
          tickSpacing: p.tickSpacing,
          hooksRegistration: p.hooksRegistrationBitmap !== void 0 ? infinitySdk.decodeHooksRegistration(p.hooksRegistrationBitmap) : void 0
        }
      };
      return viem.encodeAbiParameters(infinityRouteParamsAbi, [
        {
          poolKey: infinitySdk.encodePoolKey(poolKey),
          hookData: "0x"
        }
      ]);
    }
...
var SUCCESS_RATE_CONFIG = {
  ...
  [chains.ChainId.MONAD_MAINNET]: 0.1
};
  ...
  return function createOnChainQuoteProvider({
    ...
    const createGetRoutesWithQuotes = (isExactIn = true) => {
      ...
      return async function getRoutesWithQuote(routes, { blockNumber: blockNumberFromConfig, gasModel, retry: retryOptions, signal, quoteId }) {
        ...
        const multicallConfigs = multicallConfigsOverride?.[chainId] || chunkH57NM25L_js.BATCH_MULTICALL_CONFIGS[chainId] || chunkH57NM25L_js.BATCH_MULTICALL_CONFIGS[chains.ChainId.ETHEREUM];
        const {
...
var createMixedRouteOnChainQuoteProvider = onChainQuoteProviderFactory({
  getQuoterAddress: (chainId) => chunkH57NM25L_js.MIXED_ROUTE_QUOTER_ADDRESSES[chainId],
...
var createV3OnChainQuoteProvider = onChainQuoteProviderFactory({
  getQuoterAddress: (chainId) => chunkH57NM25L_js.V3_QUOTER_ADDRESSES[chainId],
...
var createInfinityClOnChainQuoteProvider = onChainQuoteProviderFactory({
  getQuoterAddress: (chainId) => infinitySdk.INFI_CL_QUOTER_ADDRESSES[chainId],
  getQuoteFunctionName: (isExactIn) => isExactIn ? "quoteExactInput" : "quoteExactOutput",
  abi: clQuoterAbi,
  getCallInputs: (route, isExactIn) => {
    const firstPool = route.pools[isExactIn ? 0 : route.pools.length - 1];
    const baseCurrency = swapSdkCore.getMatchedCurrency(isExactIn ? route.input : route.output, [
      firstPool.currency0,
      firstPool.currency1
    ]);
    if (!baseCurrency) {
    ...
    const exactCurrency = swapSdkCore.getCurrencyAddress(baseCurrency);
    return [
      {
        exactCurrency,
...
var createMixedRouteOnChainQuoteProviderV2 = onChainQuoteProviderFactory({
  getQuoterAddress: (chainId) => infinitySdk.INFI_MIXED_QUOTER_ADDRESSES[chainId],
...
var createInfinityBinOnChainQuoteProvider = onChainQuoteProviderFactory({
  getQuoterAddress: (chainId) => infinitySdk.INFI_BIN_QUOTER_ADDRESSES[chainId],
  getQuoteFunctionName: (isExactIn) => isExactIn ? "quoteExactInput" : "quoteExactOutput",
  abi: binQuoterAbi,
  getCallInputs: (route, isExactIn) => {
    const firstPool = route.pools[isExactIn ? 0 : route.pools.length - 1];
...
function createQuoteProvider(config) {
  ...
  const createGetRouteWithQuotes = (isExactIn = true) => {
    ...
    const createMixedRouteQuoteFetcher = (chainId) => {
      const mixedRouteOnChainQuoteProvider = infinitySdk.INFINITY_SUPPORTED_CHAINS.includes(chainId) ? mixedRouteOnChainQuoteProviderV2 : mixedRouteOnChainQuoteProviderV1;
...
var getInfinityBinPoolsWithoutBins = createOnChainPoolFactory({
  abi: infinitySdk.BinPoolManagerAbi,
  getPossiblePoolMetas: async ([currencyA, currencyB]) => {
    const { chainId } = currencyA;
    if (!infinitySdk.isInfinitySupported(chainId))
    ...
    const metas = [];
    const binPresets = chunkH57NM25L_js.BIN_PRESETS_BY_CHAIN[chainId];
    for (const { fee, binStep } of binPresets) {
      const hookPresets = chunkH57NM25L_js.BIN_HOOK_PRESETS_BY_CHAIN[chainId];
      for (const { address: hooks, registrationBitmap, poolKeyOverride } of hookPresets) {
        const poolKey = {
          ...
          parameters: {
            binStep,
            hooksRegistration: registrationBitmap !== void 0 ? infinitySdk.decodeHooksRegistration(registrationBitmap) : void 0
          },
          poolManager: infinitySdk.INFI_BIN_POOL_MANAGER_ADDRESSES[chainId],
          hooks,
          ...poolKeyOverride ?? {}
        };
        const id = infinitySdk.getPoolId(poolKey);
...
function getBinIdList({
  ...
  const price = infinitySdk.getPriceFromId(activeId, binStep);
  const [idLower, idUpper] = [
    infinitySdk.getIdFromPrice(price * (1 - priceRange), binStep),
    infinitySdk.getIdFromPrice(price * (1 + priceRange), binStep)
  ];
  const binIds = [];
...
function decodeCompactTickResult(result, pool) {
  const rawTicks = viem.decodeFunctionResult({
    abi: queryDataAbi,
...
function getTickSpacing(pool) {
  if (pool.type === 1 /* V3 */) {
    return v3Sdk.TICK_SPACINGS[Number(pool.fee)];
  }
...
async function fetchCompactPoolsTick({
  ...
  const queryHelperAddress = chunkH57NM25L_js.TICK_QUERY_HELPER_ADDRESSES[chainId];
  ...
  const res = await multicall.multicallByGasLimit(multiCallArgs, {
    chainId,
    client,
    gasLimit,
...
async function fetchTickLenPoolsTick({
  ...
  if (v3Pools.length) {
    const tickLensAddress = chunkH57NM25L_js.V3_TICK_LENS_ADDRESSES[chainId];
    if (!tickLensAddress) {
    ...
    const bitmapIndexes = v3Pools.flatMap(
      (pool) => buildV3BitmapIndexList({
        ...
        tickSpacing: v3Sdk.TICK_SPACINGS[Number(pool.fee)],
        pool
      })
    );
    if (bitmapIndexes.length) {
      ...
      for (const [index, result] of res.results.entries()) {
        ...
        const data = viem.decodeFunctionResult({
          abi: tickLensAbi,
          functionName: "getPopulatedTicksInWord",
          data: result.result
        });
  ...
  if (infinityPools.length) {
    const tickLensAddress = infinitySdk.INFI_CL_TICK_LENS_ADDRESSES[chainId];
    if (!tickLensAddress) {
...
var getInfinityClPoolsWithoutTicks = createOnChainPoolFactory({
  abi: infinitySdk.CLPoolManagerAbi,
  getPossiblePoolMetas: async ([currencyA, currencyB]) => {
    const { chainId } = currencyA;
    if (!infinitySdk.isInfinitySupported(chainId))
    ...
    const poolIdList = /* @__PURE__ */ new Set();
    const metas = [];
    const presets = chunkH57NM25L_js.CL_PRESETS_BY_CHAIN[chainId];
    const defaultHookPresets = chunkH57NM25L_js.CL_HOOK_PRESETS_BY_CHAIN[chainId];
    const whitelistLabeledHooksWithoutMetadata = (infinitySdk.whitelistLabeledHooksList[chainId] ?? []).map((hook) => ({
      address: hook,
    ...
    const addPoolMeta = (fee, tickSpacing, hooks, registrationBitmap, poolKeyOverride) => {
      const poolKey = {
        ...
        poolManager: infinitySdk.INFI_CL_POOL_MANAGER_ADDRESSES[chainId],
        hooks,
        ...poolKeyOverride ?? {}
      };
      const id = infinitySdk.getPoolId(poolKey);
  ...
  buildPool: ({ currencyA, currencyB, id, tickSpacing, poolManager, hooks, hooksRegistrationBitmap }, [liquidity, slot0]) => {
    ...
    const [currency0, currency1] = swapSdkCore.sortCurrencies([currencyA, currencyB]);
    const whitelistHook = infinitySdk.findHook(hooks, currencyA.wrapped.chainId);
    return {
      id,
      type: 3 /* InfinityCL */,
      currency0,
...
function getValidToken(chainId, token) {
  try {
    const raw = {
      address: viem.checksumAddress(token.id),
...
function parseRemotePool(remote) {
  ...
  if (remote.protocol === "v3") {
    if (remote.chainId === chains.NonEVMChainId.SOLANA)
...
function parseRemoteStablePool(remote) {
  ...
  return {
    type: 2 /* STABLE */,
    address: viem.checksumAddress(remote.id),
    balances: [swapSdkCore.CurrencyAmount.fromRawAmount(currency0, 0), swapSdkCore.CurrencyAmount.fromRawAmount(currency1, 0)],
    amplifier: 0n,
    fee: new swapSdkCore.Percent(remote.feeTier || 0, 1e3)
  };
}
function parseRemoteInfinityStablePool(remote) {
  ...
  const poolManagerAddress = infinitySdk.INFI_CL_POOL_MANAGER_ADDRESSES[chainId];
  return {
    type: 5 /* InfinityStable */,
    id: viem.checksumAddress(remote.id),
    chainId,
    currency0,
    currency1,
    fee: remote.feeTier,
    ...
    amplifier: 0,
    stableFee: 0,
    hooks: remote.hookAddress ? viem.checksumAddress(remote.hookAddress) : void 0,
...
function parseRemoteV2Pool(remote) {
  ...
  return {
    type: 0 /* V2 */,
    address: viem.checksumAddress(remote.id),
    reserve0: swapSdkCore.CurrencyAmount.fromRawAmount(currency0, 0),
    reserve1: swapSdkCore.CurrencyAmount.fromRawAmount(currency1, 0)
  };
}
function parseRemoteV3Pool(remote) {
  const { chainId } = remote;
  const isSol = chainId === chains.NonEVMChainId.SOLANA;
  return {
    type: 1 /* V3 */,
    ...
    sqrtRatioX96: 0n,
    token0ProtocolFee: new swapSdkCore.Percent(0, 1e6),
    token1ProtocolFee: new swapSdkCore.Percent(0, 1e6)
  };
}
...
function parseSolanaToken(chainId, token) {
  ...
  return new swapSdkCore.SPLToken({
    chainId,
    address: token.id,
    decimals: token.decimals,
    symbol: token.symbol,
...
function toLocalInfinityPool(remote, chainId, hooksMap) {
  ...
  let relatedHook = infinitySdk.hooksList[chainId]?.find((hook) => hook.address.toLowerCase() === hookAddress?.toLowerCase());
  ...
  const pool = {
    id: viem.checksumAddress(id),
    chainId,
    type,
    fee: feeTier,
    protocolFee,
    hooks: hookAddress ? viem.checksumAddress(hookAddress) : void 0,
    hooksRegistrationBitmap: relatedHook ? infinitySdk.encodeHooksRegistration(relatedHook.hooksRegistration) : void 0,
    poolManager: type === 4 /* InfinityBIN */ ? infinitySdk.INFI_BIN_POOL_MANAGER_ADDRESSES[chainId] : infinitySdk.INFI_CL_POOL_MANAGER_ADDRESSES[chainId],
    currency0,
    currency1,
    tvlUSD: bnTvlUsd
  };
...
var getInfinityStablePools = createOnChainPoolFactory({
  abi: infinityStableSdk.InfinityStableHook.ABI,
  ...
  getPossiblePoolMetas: async ([currencyA, currencyB], client) => {
    ...
    const hookFactoryAddress = infinityStableSdk.InfinityStableHookFactory.getHookFactoryAddress(chainId);
      ...
      if (pendingRequest) {
        ...
        const requestPromise = (async () => {
          try {
            let poolCount = 0;
            try {
              poolCount = await infinityStableSdk.InfinityStableHookFactory.getPoolCount(client, chainId);
            } catch (error) {
            ...
            const pools = await infinityStableSdk.InfinityStableHookFactory.getPoolAddressesBatch(client, chainId, poolCount);
  ...
  buildPool: ({ currencyA, currencyB, id }, [balance0, balance1, fee, amplifier]) => {
    ...
    const poolManager = infinitySdk.INFI_CL_POOL_MANAGER_ADDRESSES[currencyA.wrapped.chainId];
    const hooksRegistrationBitmap = "0x0455";
    const hooksRegistration = infinitySdk.decodeHooksRegistration(hooksRegistrationBitmap);
    const parameters = {
      tickSpacing: DEFAULT_TICK_SPACING,
      hooksRegistration
    };
    const poolId = infinitySdk.getPoolId({
    ...
    return {
      ...
      reserve0: sdk.CurrencyAmount.fromRawAmount(currency0, balance0.toString()),
      reserve1: sdk.CurrencyAmount.fromRawAmount(currency1, balance1.toString()),
      tickSpacing: DEFAULT_TICK_SPACING,
      poolManager,
      hooks: id,
      hooksRegistrationBitmap,
...
var schema_exports = {};
chunkH57NM25L_js.__export(schema_exports, {
  zPools: () => zPools,
...
var zHooksRegistrationBitmap = zod.z.number().or(zHex);
...
var zInfinityClPool = zod.z.object({
  type: zod.z.literal(3 /* InfinityCL */),
  currency0: zCurrency,
  currency1: zCurrency,
  fee: zFee,
  liquidity: zBigNumber,
...
var zBinReserves = zod.z.object({
  reserveX: zBigNumber,
  reserveY: zBigNumber
}).required();
var zInfinityBinPool = zod.z.object({
  type: zod.z.literal(4 /* InfinityBIN */),
  currency0: zCurrency,
  currency1: zCurrency,
  fee: zFee,
  ...
  hooks: zAddress.optional(),
  hooksRegistrationBitmap: zHooksRegistrationBitmap.optional(),
  reserveOfBin: zod.z.record(zBigNumber, zBinReserves).optional()
});
var zInfinityStablePool = zod.z.object({
  type: zod.z.literal(5 /* InfinityStable */),
  currency0: zCurrency,
  currency1: zCurrency,
  fee: zFee,
  tickSpacing: zod.z.number(),
  ...
  reserve1: zCurrencyAmountOptional,
  stableFee: zFee,
  amplifier: zod.z.number()
});
var zPools = zod.z.array(
  zod.z.union([zV3Pool, zV2Pool, zStablePool, zInfinityClPool, zInfinityBinPool, zInfinityStablePool])
);
...
function estimateGasCost({ pool, poolAfter }) {
  if (isV2Pool(pool)) {
    return chunkH57NM25L_js.BASE_SWAP_COST_V2;
  }
  if (isV3Pool(pool) && isV3Pool(poolAfter)) {
    ...
    const numOfTicksCrossed = v3Sdk.TickList.countInitializedTicksCrossed(ticks, tick, tickAfter);
    const tickGasUse = chunkH57NM25L_js.COST_PER_INIT_TICK(chainId) * BigInt(numOfTicksCrossed);
    return chunkH57NM25L_js.BASE_SWAP_COST_V3(chainId) + chunkH57NM25L_js.COST_PER_HOP_V3(chainId) + tickGasUse;
  }
  if (isStablePool(pool)) {
    return chunkH57NM25L_js.BASE_SWAP_COST_STABLE_SWAP;
  }
...
chunkH57NM25L_js.__export(transformer_exports2, {
...
chunkH57NM25L_js.__export(smartRouter_exports, {
    ...
    for (const pool of pools) {
      ...
      if (isV2Pool(pool)) {
        if (!poolTypeSet.has(type)) {
          baseGasUse += chunkH57NM25L_js.BASE_SWAP_COST_V2;
          poolTypeSet.add(type);
          continue;
        }
        baseGasUse += chunkH57NM25L_js.COST_PER_EXTRA_HOP_V2;
        continue;
      }
      if (isV3Pool(pool)) {
        if (!poolTypeSet.has(type)) {
          baseGasUse += chunkH57NM25L_js.BASE_SWAP_COST_V3(chainId);
          poolTypeSet.add(type);
        }
        baseGasUse += chunkH57NM25L_js.COST_PER_HOP_V3(chainId);
        continue;
      }
      if (isStablePool(pool) || isInfinityStablePool(pool)) {
        if (!poolTypeSet.has(type)) {
          baseGasUse += chunkH57NM25L_js.BASE_SWAP_COST_STABLE_SWAP;
          poolTypeSet.add(type);
          continue;
        }
        baseGasUse += chunkH57NM25L_js.COST_PER_EXTRA_HOP_STABLE_SWAP;
        continue;
      }
    }
    const tickGasUse = chunkH57NM25L_js.COST_PER_INIT_TICK(chainId) * totalInitializedTicksCrossed;
    const uninitializedTickGasUse = chunkH57NM25L_js.COST_PER_UNINIT_TICK * 0n;
...
async function getBestTrade3(amount, currency, tradeType, config) {
  ...
  logger3.debug(
    `[SmartRouter] getBestTrade ${config.quoteId}, [${sdk.TradeType[tradeType]}] input=${amount.toExact()} ${currency.symbol} maxSplits=${config.maxSplits} maxHops=${config.maxHops}`
  );
  try {
...
var zHooksRegistrationBitmap2 = zod.z.number().or(zHex2);
...
var zCurrencyAmount2 = zod.z.object({
...
var zInfinityClPool2 = zod.z.object({
  type: zod.z.literal(3 /* InfinityCL */),
...
var zInfinityBinPool2 = zod.z.object({
  type: zod.z.literal(4 /* InfinityBIN */),
  currency0: zCurrency2,
  currency1: zCurrency2,
  fee: zFee2,
...
var zInfinityStablePool2 = zod.z.object({
  type: zod.z.literal(5 /* InfinityStable */),
  currency0: zCurrency2,
  currency1: zCurrency2,
  fee: zFee2,
...
var zPools2 = zod.z.array(
  zod.z.union([zV2Pool2, zV3Pool2, zStablePool2, zInfinityClPool2, zInfinityBinPool2, zInfinityStablePool2])
);
...
Object.defineProperty(exports, 'ADDITIONAL_BASES', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.ADDITIONAL_BASES; }
});
Object.defineProperty(exports, 'ADDRESS_THIS', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.ADDRESS_THIS; }
});
Object.defineProperty(exports, 'BASES_TO_CHECK_TRADES_AGAINST', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BASES_TO_CHECK_TRADES_AGAINST; }
});
Object.defineProperty(exports, 'BASE_SWAP_COST_STABLE_SWAP', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BASE_SWAP_COST_STABLE_SWAP; }
});
Object.defineProperty(exports, 'BASE_SWAP_COST_V2', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BASE_SWAP_COST_V2; }
});
Object.defineProperty(exports, 'BASE_SWAP_COST_V3', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BASE_SWAP_COST_V3; }
});
Object.defineProperty(exports, 'BATCH_MULTICALL_CONFIGS', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BATCH_MULTICALL_CONFIGS; }
});
Object.defineProperty(exports, 'BETTER_TRADE_LESS_HOPS_THRESHOLD', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BETTER_TRADE_LESS_HOPS_THRESHOLD; }
});
Object.defineProperty(exports, 'BIG_INT_TEN', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BIG_INT_TEN; }
});
Object.defineProperty(exports, 'BIN_HOOK_PRESETS_BY_CHAIN', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BIN_HOOK_PRESETS_BY_CHAIN; }
});
Object.defineProperty(exports, 'BIN_PRESETS_BY_CHAIN', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BIN_PRESETS_BY_CHAIN; }
});
Object.defineProperty(exports, 'BIPS_BASE', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.BIPS_BASE; }
});
Object.defineProperty(exports, 'CL_HOOK_PRESETS_BY_CHAIN', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.CL_HOOK_PRESETS_BY_CHAIN; }
});
Object.defineProperty(exports, 'CL_PRESETS', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.CL_PRESETS; }
});
Object.defineProperty(exports, 'CL_PRESETS_BY_CHAIN', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.CL_PRESETS_BY_CHAIN; }
});
Object.defineProperty(exports, 'COST_PER_EXTRA_HOP_STABLE_SWAP', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.COST_PER_EXTRA_HOP_STABLE_SWAP; }
});
Object.defineProperty(exports, 'COST_PER_EXTRA_HOP_V2', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.COST_PER_EXTRA_HOP_V2; }
});
Object.defineProperty(exports, 'COST_PER_HOP_V3', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.COST_PER_HOP_V3; }
});
Object.defineProperty(exports, 'COST_PER_INIT_TICK', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.COST_PER_INIT_TICK; }
});
Object.defineProperty(exports, 'COST_PER_UNINIT_TICK', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.COST_PER_UNINIT_TICK; }
});
Object.defineProperty(exports, 'CUSTOM_BASES', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.CUSTOM_BASES; }
});
Object.defineProperty(exports, 'EMPTY_HOOK', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.EMPTY_HOOK; }
});
Object.defineProperty(exports, 'InfinityFeeTier', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.InfinityFeeTier; }
});
Object.defineProperty(exports, 'MIN_BNB', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.MIN_BNB; }
});
Object.defineProperty(exports, 'MIXED_ROUTE_QUOTER_ADDRESSES', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.MIXED_ROUTE_QUOTER_ADDRESSES; }
});
Object.defineProperty(exports, 'MSG_SENDER', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.MSG_SENDER; }
});
Object.defineProperty(exports, 'SMART_ROUTER_ADDRESSES', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.SMART_ROUTER_ADDRESSES; }
});
Object.defineProperty(exports, 'STABLE_SWAP_INFO_ADDRESS', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.STABLE_SWAP_INFO_ADDRESS; }
});
Object.defineProperty(exports, 'TICK_QUERY_HELPER_ADDRESSES', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.TICK_QUERY_HELPER_ADDRESSES; }
});
Object.defineProperty(exports, 'V2_FEE_PATH_PLACEHOLDER', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.V2_FEE_PATH_PLACEHOLDER; }
});
Object.defineProperty(exports, 'V2_ROUTER_ADDRESS', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.V2_ROUTER_ADDRESS; }
});
Object.defineProperty(exports, 'V3_QUOTER_ADDRESSES', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.V3_QUOTER_ADDRESSES; }
});
Object.defineProperty(exports, 'V3_TICK_LENS_ADDRESSES', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.V3_TICK_LENS_ADDRESSES; }
});
Object.defineProperty(exports, 'usdGasTokensByChain', {
  enumerable: true,
  get: function () { return chunkH57NM25L_js.usdGasTokensByChain; }
});
exports.InfinityMixedQuoterActions = InfinityMixedQuoterActions;
exports.InfinityRouter = infinity_router_exports;
