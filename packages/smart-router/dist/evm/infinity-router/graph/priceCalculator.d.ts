import { Currency, CurrencyAmount, Price } from '@pancakeswap/sdk';
import { Graph, Vertice } from '../types';
type Params = {
    graph: Graph;
    quote: Vertice;
    gasPriceWei: bigint;
};
export type PriceCalculator = ReturnType<typeof createPriceCalculator>;
export declare function createPriceCalculator({ graph, quote, gasPriceWei }: Params): {
    graph: Graph;
    getGasPriceInBase: (base: Vertice) => CurrencyAmount<Currency> | undefined;
    getQuotePrice: (base: Vertice) => Price<Currency, Currency> | undefined;
    getPrice: (base: Vertice, targetQuote: Vertice) => Price<Currency, Currency> | undefined;
};
export {};
//# sourceMappingURL=priceCalculator.d.ts.map