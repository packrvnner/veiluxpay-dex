...
var first__default = /*#__PURE__*/_interopDefault(first);
...
var UNIVERSAL_ROUTER_ADDRESSES = {
  ...
  [chains.ChainId.SEPOLIA]: "0x6A3d58cc25a92d90Fa3a8f0f5d5e75AD01ccd7a6",
  [chains.ChainId.BSC]: "0xd9C500DfF816a1Da21A48A732d3498Bf09dc9AEB",
  [chains.ChainId.BSC_TESTNET]: "0x87FD5305E6a40F378da124864B2D479c2028BD86",
  ...
  [chains.ChainId.BASE]: "0xd9C500DfF816a1Da21A48A732d3498Bf09dc9AEB",
  ...
  [chains.ChainId.MONAD_MAINNET]: "0x23682a588CF2601ACa977dF200938634c9F7d552",
...
function currencyAddressInfinity(currency) {
  return currency.isNative ? sdk.ZERO_ADDRESS : currency.wrapped.address;
}
var getPoolKey = (pool) => {
  const base = {
  ...
  if (infinitySdk.isDynamicFeeHook(chainId, pool.hooks)) {
    base.fee = infinitySdk.DYNAMIC_FEE_FLAG;
  }
  ...
  if (smartRouter.SmartRouter.isInfinityClPool(pool) || smartRouter.SmartRouter.isInfinityStablePool(pool)) {
    const params = {
  ...
  if (smartRouter.SmartRouter.isInfinityBinPool(pool)) {
    const params = {
...
function getCurrencies(pool) {
  if (smartRouter.SmartRouter.isV2Pool(pool)) {
    const { reserve0, reserve1 } = pool;
    return [reserve0.currency, reserve1.currency];
  }
  if (smartRouter.SmartRouter.isV3Pool(pool)) {
    const { token0, token1 } = pool;
    return [token0, token1];
  }
  if (smartRouter.SmartRouter.isStablePool(pool)) {
    const { balances } = pool;
    return balances.map((b) => b.currency);
  }
  if (smartRouter.SmartRouter.isInfinityClPool(pool) || smartRouter.SmartRouter.isInfinityBinPool(pool) || smartRouter.SmartRouter.isInfinityStablePool(pool)) {
...
var ABI_PARAMETER = {
  ...
  [16 /* INFI_SWAP */]: viem.parseAbiParameters("bytes actions, bytes[] params")
};
...
var parseSwapTradeContext = (trade, options) => {
  const context = {
    ...
    user: options.recipient || smartRouter.MSG_SENDER,
    returnChanges: null,
  ...
  for (let k = 0; k < trade.routes.length; k++) {
    ...
    const amountIn = smartRouter.SmartRouter.maximumAmountIn(trade, options.slippageTolerance, route.inputAmount);
    const amountOut = smartRouter.SmartRouter.minimumAmountOut(trade, options.slippageTolerance, route.outputAmount);
    const routeContext = {
      sections: [],
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      invariant2__default.default(section.length, "EMPTY_SECTION");
      const type = poolTypeToRouteType(section[0].type);
      invariant2__default.default(
        type === smartRouter.RouteType.V2 || type === smartRouter.RouteType.V3 || type === smartRouter.RouteType.InfinityBIN || type === smartRouter.RouteType.InfinityCL || type === smartRouter.RouteType.STABLE,
        "INVALID_ROUTE_TYPE"
      );
      const isFirstSection = i === 0;
      const isLastSection = i === sections.length - 1;
      const poolIn = getInputCurrency(section[0], inputCurrency);
      const poolOut = smartRouter.SmartRouter.getOutputOfPools(section, poolIn);
      const output = isLastSection ? trade.outputAmount.currency : poolOut;
      const newRoute = smartRouter.SmartRouter.buildBaseRoute([...section], poolIn, poolOut);
      const swap = {
        type,
        poolIn,
        poolOut,
        ...
        pools: section,
        route: newRoute,
        isInfinity: section[0].type === smartRouter.PoolType.InfinityBIN || section[0].type === smartRouter.PoolType.InfinityCL || section[0].type === smartRouter.PoolType.InfinityStable
      };
...
function poolTypeToRouteType(poolType) {
  switch (poolType) {
    case smartRouter.PoolType.V2:
      return smartRouter.RouteType.V2;
    case smartRouter.PoolType.V3:
      return smartRouter.RouteType.V3;
    case smartRouter.PoolType.STABLE:
      return smartRouter.RouteType.STABLE;
    case smartRouter.PoolType.InfinityBIN:
      return smartRouter.RouteType.InfinityBIN;
    case smartRouter.PoolType.InfinityCL:
    case smartRouter.PoolType.InfinityStable:
      return smartRouter.RouteType.InfinityCL;
    default:
      throw new Error("Invalid pool type");
  }
}
function partition(route) {
  ...
  for (const pool of route.pools) {
    const outputCurrency = smartRouter.SmartRouter.getOutputCurrency(pool, inputCurrency);
...
var TradePlanner = class extends RoutePlanner {
  ...
  addMergedWrapBeforeTrade() {
    if (this.context.mergedWrapBeforeTrade) {
      ...
      const recipient = smartRouter.ADDRESS_THIS;
      const amount = smartRouter.SmartRouter.maximumAmountIn(this.trade, this.options.slippageTolerance).quotient;
      if (wrap) {
  ...
  addSwapCommand(route, section) {
    ...
    const lastSection = last__default.default(route.sections);
    ...
    const nextIsV2 = Boolean(nextSection && nextSection.type === smartRouter.RouteType.V2);
    const nextRouteAddress = relaxToV2(nextIsV2, nextSection ? nextSection.pools[0] : null);
    const nextRecipient = nextSection?.wrapInput || nextSection?.unwrapInput ? smartRouter.ADDRESS_THIS : nextRouteAddress;
    ...
    switch (section.type) {
      ...
      case smartRouter.RouteType.V3: {
        ...
        const path = smartRouter.SmartRouter.encodeMixedRouteToPath(
          ...
          tradeType === sdk.TradeType.EXACT_OUTPUT
        );
        if (tradeType === sdk.TradeType.EXACT_INPUT) {
          const params = [
            recipient,
      ...
      case smartRouter.RouteType.InfinityBIN:
      case smartRouter.RouteType.InfinityCL: {
        if (!this.context.reusedActionPlaner) {
          this.context.reusedActionPlaner = new infinitySdk.ActionsPlanner();
        }
  ...
  addInfinityMultiHop(actionPlaner, section, amountIn, amountOut) {
    ...
    const isCL = smartRouter.SmartRouter.isInfinityClPool(pool) || smartRouter.SmartRouter.isInfinityStablePool(pool);
    if (tradeType === sdk.TradeType.EXACT_INPUT) {
      const params = {
        currencyIn: currencyAddressInfinity(section.poolIn),
        path: smartRouter.SmartRouter.encodeInfinityRouteToPath(section.route, false),
        amountIn,
        amountOutMinimum: amountOut
      };
      actionPlaner.add(isCL ? infinitySdk.ACTIONS.CL_SWAP_EXACT_IN : infinitySdk.ACTIONS.BIN_SWAP_EXACT_IN, [params]);
    } else {
      const params = {
        currencyOut: currencyAddressInfinity(section.poolOut),
        path: smartRouter.SmartRouter.encodeInfinityRouteToPath(section.route, true),
        amountOut,
        amountInMaximum: amountIn
      };
      actionPlaner.add(isCL ? infinitySdk.ACTIONS.CL_SWAP_EXACT_OUT : infinitySdk.ACTIONS.BIN_SWAP_EXACT_OUT, [params]);
    }
  }
  addInfinitySingleHop(actionPlaner, section, amountIn, amountOut) {
    const pool = section.pools[0];
    const poolKey = getPoolKey(pool);
    const _encodedPoolKey = infinitySdk.encodePoolKey(poolKey);
    const { tradeType } = this.context.trade;
    ...
    if (tradeType === sdk.TradeType.EXACT_INPUT) {
      ...
      actionPlaner.add(isCL ? infinitySdk.ACTIONS.CL_SWAP_EXACT_IN_SINGLE : infinitySdk.ACTIONS.BIN_SWAP_EXACT_IN_SINGLE, [params]);
    } else {
      const params = {
        ...baseParams,
        amountOut,
      ...
      actionPlaner.add(isCL ? infinitySdk.ACTIONS.CL_SWAP_EXACT_OUT_SINGLE : infinitySdk.ACTIONS.BIN_SWAP_EXACT_OUT_SINGLE, [params]);
    }
  }
  addWrapBeforeSwap(route, section) {
    ...
    const forwardV2 = !section.isFirstSection && Boolean(nextSection && nextSection.type === smartRouter.RouteType.V2);
    ...
    if (section.unwrapInput) {
      ...
      if (section.payerIsUser) {
        this.addCommand(2 /* PERMIT2_TRANSFER_FROM */, [section.route.input.wrapped.address, smartRouter.ADDRESS_THIS, amount]);
      }
      this.addCommand(12 /* UNWRAP_WETH */, [recipient, amount]);
    }
  }
  addWrapAfterRoute(route) {
    ...
    const lastSection = last__default.default(sections);
    const firstSection = first__default.default(sections);
    const { mustCustody, trade } = this.context;
    const { tradeType } = trade;
    if (this.context.returnChanges === null && tradeType === sdk.TradeType.EXACT_OUTPUT && (firstSection.wrapInput || firstSection.unwrapInput)) {
      this.context.returnChanges = { wrap: !firstSection.wrapInput };
    }
    if (!lastSection.wrapOutput && !lastSection.unwrapOutput) {
      return;
    }
    const recipient = mustCustody ? smartRouter.ADDRESS_THIS : this.context.user;
  ...
  addWrapOut(recipient, amountMinimum, currency) {
    this.addCommand(11 /* WRAP_ETH */, [smartRouter.ADDRESS_THIS, BigInt(ACTION_CONSTANTS.CONTRACT_BALANCE)]);
  ...
  payFee() {
    let outAmount = smartRouter.SmartRouter.minimumAmountOut(this.trade, this.context.options.slippageTolerance);
    const { tradeType } = this.context.trade;
    if (this.context.takeOverWrapSweep) {
      ...
      outAmount = sdk.CurrencyAmount.fromRawAmount(outputCurrency, outAmount.quotient);
    }
  ...
  initializeInfinitySwap(actionPlaner, section, route) {
    ...
    actionPlaner.add(infinitySdk.ACTIONS.SETTLE, [
  ...
  finalizeInfinitySwap(actionPlaner, section, recipient, amountIn) {
    ...
    if (!mergePrev) {
      if (payerIsUser) {
        actionPlaner.add(infinitySdk.ACTIONS.SETTLE, [currencyAddressInfinity(section.poolIn), amountIn, true]);
      }
    }
    if (!mergeNext) {
      actionPlaner.add(infinitySdk.ACTIONS.TAKE, [currencyAddressInfinity(section.poolOut), recipient, ACTION_CONSTANTS.OPEN_DELTA]);
      if (this.trade.tradeType === sdk.TradeType.EXACT_OUTPUT) {
        actionPlaner.add(infinitySdk.ACTIONS.TAKE, [
          currencyAddressInfinity(section.poolIn),
          recipient,
...
function relaxToV2(forwardV2, pool) {
  const nextRouteAddress = forwardV2 ? smartRouter.getPoolAddress(pool) : smartRouter.ADDRESS_THIS;
  if (!nextRouteAddress) {
...
function encodeInputTokenOptions(planner, options) {
  if (!!options.approval && !!options.permit2Permit)
    invariant2__default.default(options.approval.token === options.permit2Permit.details.token, `inconsistent token`);
  if (!!options.approval && !!options.permit2TransferFrom)
    invariant2__default.default(options.approval.token === options.permit2TransferFrom.token, `inconsistent token`);
  if (!!options.permit2TransferFrom && !!options.permit2Permit)
    invariant2__default.default(options.permit2TransferFrom.token === options.permit2Permit.details.token, `inconsistent token`);
...
var UnwrapWETH = class {
  constructor(amount, chainId, permit2) {
    ...
    if (permit2) {
      ...
      invariant2__default.default(permit2.details.amount >= amount, `Did not permit enough WETH for unwrapWETH transaction`);
  ...
  encode(planner, _) {
    ...
    planner.addCommand(12 /* UNWRAP_WETH */, [smartRouter.ADDRESS_THIS, BigInt(this.amount.toString())]);
  }
};
...
function decodeUniversalCalldata(calldata, abiParameter = ABI_PARAMETER) {
  const { functionName, args } = viem.decodeFunctionData({
  ...
  for (const [index, command] of Object.entries(commands)) {
    ...
    const parameters = viem.decodeAbiParameters(abi, args[1][Number(index)]);
    const formatedArgs = [];
...
function decodeActions(actionsHex, inputs) {
  const actions = Array.from(viem.toBytes(actionsHex));
  const decodedActions = [];
  for (const [actionIndex, actionCode] of Object.entries(actions)) {
    const actionType = actionCode;
    const actionName = infinitySdk.ACTIONS[actionType];
    const abi = infinitySdk.ACTIONS_ABI[actionType];
    const parameters = viem.decodeAbiParameters(abi, inputs[Number(actionIndex)]);
    const formatedArgs = [];
    for (const [i, p] of Object.entries(abi)) {
      formatedArgs.push({
        type: p.type,
...
var PancakeSwapUniversalRouter = class {
  ...
  static swapERC20CallParameters(trade, _options) {
    ...
    if (options.payerIsUser) {
      invariant2__default.default(!(inputCurrency.isNative && !!options.inputTokenPermit), "NATIVE_INPUT_PERMIT");
    }
    if (options.inputTokenPermit && typeof options.inputTokenPermit === "object") {
      encodePermit(planner, options.inputTokenPermit);
    }
    const nativeCurrencyValue = options.payerIsUser ? inputCurrency.isNative ? smartRouter.SmartRouter.maximumAmountIn(planner.trade, options.slippageTolerance, planner.trade.inputAmount).quotient : 0n : 0n;
    planner.encode();
