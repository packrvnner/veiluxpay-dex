...
import { ChainId as ChainId$1 } from '@pancakeswap/sdk';
...
var HOOK_INFINITY_STABLE_HOOK_FACTORY_ADDRESS = {
  ...
  [ChainId.BSC_TESTNET]: "0x9188584835110FB6e0eB3BAE10ef1459Acf99edB"
...
var _InfinityStablePoolFactory = class {
  ...
  static encodeCreatePool(options) {
    ...
    invariant(options.A !== void 0, "A parameter is required");
    invariant(options.fee !== void 0, "fee parameter is required");
    invariant(options.offpegFeeMultiplier !== void 0, "offpegFeeMultiplier parameter is required");
    invariant(options.maExpTime !== void 0, "maExpTime parameter is required");
    invariant(options.assetTypes !== void 0, "assetTypes parameter is required");
    const { A } = options;
    const { fee } = options;
  ...
  static encodeCreatePoolAndAddLiquidity(options) {
    ...
    invariant(options.maExpTime !== void 0, "maExpTime parameter is required");
    invariant(options.assetTypes !== void 0, "assetTypes parameter is required");
    invariant(options.amount0 !== void 0, "amount0 is required");
    invariant(options.amount1 !== void 0, "amount1 is required");
    invariant(options.minMintAmount !== void 0, "minMintAmount is required");
    invariant(options.receiver !== void 0, "receiver is required");
    const { A } = options;
    const { fee } = options;
    const { offpegFeeMultiplier } = options;
    const { maExpTime } = options;
...
function validateFee(fee) {
  ...
  invariant(fee <= 10000000n, "FEE_TOO_HIGH");
}
...
var INFINITY_STABLE_SUPPORTED_CHAINS = [ChainId$1.BSC, ChainId$1.BSC_TESTNET];
...
export { ADDRESS_ZERO, CL_STABLE_SWAP_POOL_FACTORY_ADDRESS, DEFAULT_ASSET_TYPE, DEFAULT_IMPLEMENTATION_IDX, HOOK_INFINITY_STABLE_HOOK_FACTORY_ADDRESS, INFINITY_STABLE_POOL_FEE_DENOMINATOR, InfinityStableHook, InfinityStableHookFactory, InfinityStablePoolFactory, MAX_A, MAX_FEE, MIN_A, NULL_METHOD_ID, PRESET_CONFIGS, TokenType, feeToPercentage, generatePoolName, generatePoolSymbol, getHookAddressFromReceipt, getPoolIdFromReceipt, getSortedTokenAddresses, infinityStablePoolFactoryABI, isInfinityStableSupported, percentageToFee, sortCurrencies2 as sortCurrencies, tokenConfigsToArrays, validateAmplification, validateCurrency, validateCurrencyPair, validateFee };
