...
var routingSdkAddonInfinity = require('@pancakeswap/routing-sdk-addon-infinity');
...
function isInfinityCLRoute(r) {
  const { pools } = r;
  return pools.every((p) => routingSdkAddonInfinity.isInfinityCLPool(p));
}
function isInfinityBinRoute(r) {
  const { pools } = r;
  return pools.every((p) => routingSdkAddonInfinity.isInfinityBinPool(p));
}
function isInfinityStableRoute(r) {
  const { pools } = r;
  return pools.every((p) => routingSdkAddonInfinity.isInfinityStablePool(p));
}
function isMixedRoute(r) {
  const { pools } = r;
  const hasInfinityPool = pools.some((p) => routingSdkAddonInfinity.isInfinityCLPool(p) || routingSdkAddonInfinity.isInfinityStablePool(p));
  if (hasInfinityPool) {
    return false;
  }
  let lastType;
...
function isInfinityMixedRoute(r) {
  const { pools } = r;
  const hasInfinityPool = pools.some((p) => routingSdkAddonInfinity.isInfinityCLPool(p) || routingSdkAddonInfinity.isInfinityBinPool(p) || routingSdkAddonInfinity.isInfinityStablePool(p));
  if (!hasInfinityPool) {
    return false;
  }
  let lastType;
...
function encodeInfinityMixedRouteActions(route) {
  return viem.bytesToHex(
    new Uint8Array(
      route.pools.map((p) => {
        if (routingSdkAddonV2.isV2Pool(p)) {
          return 2 /* V2_EXACT_INPUT_SINGLE */;
        }
        if (routingSdkAddonV3.isV3Pool(p)) {
          return 3 /* V3_EXACT_INPUT_SINGLE */;
        }
        if (routingSdkAddonStableSwap.isStablePool(p)) {
          return 0 /* SS_2_EXACT_INPUT_SINGLE */;
        }
        if (routingSdkAddonInfinity.isInfinityCLPool(p) || routingSdkAddonInfinity.isInfinityStablePool(p)) {
          return 4 /* INFI_CL_EXACT_INPUT_SINGLE */;
        }
        if (routingSdkAddonInfinity.isInfinityBinPool(p)) {
...
function getOutputCurrency(pool, currencyIn) {
  ...
  if (routingSdkAddonInfinity.isInfinityCLPool(pool) || routingSdkAddonInfinity.isInfinityBinPool(pool) || routingSdkAddonInfinity.isInfinityStablePool(pool)) {
...
function encodeInfinityMixedRouteCurrencyPath(route) {
  ...
  const pathStart = swapSdkCore.getMatchedCurrency(currencyStart, pools[0].getTradingPairs()[0]);
  if (!pathStart) {
...
function encodeInfinityMixedRouteParams(route) {
  return route.pools.map((p) => {
    if (routingSdkAddonV2.isV2Pool(p) || routingSdkAddonStableSwap.isStablePool(p)) {
      return "0x";
    }
    if (routingSdkAddonV3.isV3Pool(p)) {
      return viem.encodeAbiParameters(viem.parseAbiParameters("uint24"), [p.getPoolData().fee]);
    }
    if (routingSdkAddonInfinity.isInfinityCLPool(p) || routingSdkAddonInfinity.isInfinityStablePool(p)) {
      const pool = p.getPoolData();
      const poolKey = {
        currency0: swapSdkCore.getCurrencyAddress(pool.currency0),
        currency1: swapSdkCore.getCurrencyAddress(pool.currency1),
        fee: infinitySdk.isDynamicFeeHook(pool.currency0.chainId, pool.hooks) ? infinitySdk.DYNAMIC_FEE_FLAG : pool.fee,
        hooks: pool.hooks,
        poolManager: pool.poolManager,
        parameters: {
          tickSpacing: pool.tickSpacing,
          hooksRegistration: pool.hooksRegistrationBitmap !== void 0 ? infinitySdk.decodeHooksRegistration(pool.hooksRegistrationBitmap) : void 0
        }
      };
...
function encodeInfinityRouteToPath(route, exactOutput) {
  if (route.pools.some((p) => !routingSdkAddonInfinity.isInfinityCLPool(p) && !routingSdkAddonInfinity.isInfinityBinPool(p) && !routingSdkAddonInfinity.isInfinityStablePool(p))) {
  ...
  const { path } = pools.reduce(
    ({ baseCurrency, path: path2 }, p) => {
      const isInfinityCl = routingSdkAddonInfinity.isInfinityCLPool(p);
      const isInfinityBin = routingSdkAddonInfinity.isInfinityBinPool(p);
      const isInfinityStable = routingSdkAddonInfinity.isInfinityStablePool(p);
...
function buildInfinityMixedQuoteCall(route) {
  ...
  return {
    address: infinitySdk.INFI_MIXED_QUOTER_ADDRESSES[chainId],
...
function buildInfinityCLQuoteCall(route) {
  ...
  const baseCurrency = swapSdkCore.getMatchedCurrency(route.amount.currency, firstPool.getTradingPairs()[0]);
  if (!baseCurrency) {
  ...
  return {
    address: infinitySdk.INFI_CL_QUOTER_ADDRESSES[chainId],
    abi: clQuoterAbi,
    functionName: isExactOut ? "quoteExactOutput" : "quoteExactInput",
    args: [
      {
...
function buildInfinityBinQuoteCall(route) {
  ...
  const exactCurrency = swapSdkCore.getCurrencyAddress(baseCurrency);
  return {
    address: infinitySdk.INFI_BIN_QUOTER_ADDRESSES[chainId],
    abi: binQuoterAbi,
    functionName: isExactOut ? "quoteExactOutput" : "quoteExactInput",
    args: [
      {
...
var fetchQuotes = async ({ routes, client }) => {
  ...
  return results.map((result, i) => {
    ...
    if (result.result.length === 2) {
      ...
      return {
        quote: swapSdkCore.CurrencyAmount.fromRawAmount(outCurrency, quote2),
...
Object.defineProperty(exports, 'INFI_BIN_QUOTER_ADDRESSES', {
  enumerable: true,
  get: function () { return infinitySdk.INFI_BIN_QUOTER_ADDRESSES; }
});
Object.defineProperty(exports, 'INFI_CL_QUOTER_ADDRESSES', {
  enumerable: true,
  get: function () { return infinitySdk.INFI_CL_QUOTER_ADDRESSES; }
});
Object.defineProperty(exports, 'INFI_MIXED_QUOTER_ADDRESSES', {
  enumerable: true,
  get: function () { return infinitySdk.INFI_MIXED_QUOTER_ADDRESSES; }
});
...
exports.isInfinityBinRoute = isInfinityBinRoute;
exports.isInfinityCLRoute = isInfinityCLRoute;
exports.isInfinityMixedRoute = isInfinityMixedRoute;
exports.isInfinityStableRoute = isInfinityStableRoute;
