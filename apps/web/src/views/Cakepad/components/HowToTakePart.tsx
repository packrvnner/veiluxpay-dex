...
const HowToTakePart: React.FC = () => {
  ...
  return (
    <SectionWrapper id="launchpad-how-to">
      ...
      >
        <StepCard
          ...
          items={[
            ...
            t('When the VLX.PAD event is live, simply commit your %symbol% tokens.', { symbol: commitTokensText }),
            t(
        ...
        <StepCard
          ...
          circleFill="secondary"
          items={[
            t('After the VLX.PAD event ends, you can claim the VLX.PAD tokens you purchased.'),
            t(
              'If the VLX.PAD event is oversubscribed (total committed amount > raise goal), unspent %symbol% will be refunded.',
              {
                symbol: commitTokensText,
              },
            ),
            t(
              'A participation fee (0.05%–1%) applies only when the VLX.PAD event is oversubscribed. The fee decreases as the oversubscription % rises and is deducted from your refunded %symbol%.',
              { symbol: commitTokensText },
            ),
          ]}
        />
