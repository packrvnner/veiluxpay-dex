...
export const FarmV3Card: React.FC<React.PropsWithChildren<FarmCardProps>> = ({ farm, removed, account }) => {
  ...
  const isPromotedFarm = farm.token.symbol === 'VLX'
  ...
  const aprTooltip = useTooltip(
    <>
      <Text>
        {t(
          'Global APR calculated using the total amount of active & staked liquidity with the pool VLX reward emissions.',
        )}
      </Text>
      <br />
