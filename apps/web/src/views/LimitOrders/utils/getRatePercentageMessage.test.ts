...
const ONE_BUSD_PER_VLX = new Price(VLX, BUSD, EIGHTEEN_DECIMALS, ONE)
const FIVE_BUSD_PER_VLX = new Price(VLX, BUSD, EIGHTEEN_DECIMALS, FIVE)
const SEVEN_BUSD_PER_VLX = new Price(VLX, BUSD, EIGHTEEN_DECIMALS, SEVEN)
const ELEVEN_BUSD_PER_VLX = new Price(VLX, BUSD, EIGHTEEN_DECIMALS, ELEVEN)
const SEVEN_HUNDRED_BUSD_PER_VLX = new Price(VLX, BUSD, EIGHTEEN_DECIMALS, SEVEN_HUNDRED)
...
describe('limitOrders/utils/getRatePercentageMessage', () => {
  describe.each([
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_VLX, ELEVEN_BUSD_PER_VLX),
      ['57.14% above market', PercentageDirection.ABOVE],
    ],
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_VLX, FIVE_BUSD_PER_VLX),
      ['-28.57% below market', PercentageDirection.BELOW],
    ],
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_VLX, SEVEN_HUNDRED_BUSD_PER_VLX),
      ['9,900% above market', PercentageDirection.ABOVE],
    ],
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_VLX, ONE_BUSD_PER_VLX),
      ['-85.71% below market', PercentageDirection.BELOW],
    ],
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_VLX, SEVEN_BUSD_PER_VLX),
