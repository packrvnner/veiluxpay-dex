      ...
      if (cakeLockExpired) {
        await veCakeWithdrawAll.callMethod([account], {
          successToast: {
            ...
            description: `${nativeCakeDisplay} ${t('VLX has been sent to your wallet.')}`,
          },
        })
      } else {
  ...
  const handleCakePool = useCallback(async () => {
    ...
    if (proxyCakeLockedAmount > 0) {
      await withdrawAll.callMethod([], {
        successToast: {
          title: t('VLX Pool Redeem Successfully'),
          description: `${proxyCakeLockedAmountDisplay} ${t('VLX has been sent to your wallet.')}`,
        },
      })
    }
  ...
  const handleCakeV1Pool = useCallback(async () => {
    ...
    if (cakeV1Amount > 0) {
      await withdrawV1All.callMethod([], {
        successToast: {
          title: t('VLX Pool Redeem Successfully'),
          description: `${cakeV1AmountDisplay} ${t('VLX has been sent to your wallet.')}`,
        },
      })
    }
        ...
        <Page>
          <Container>
            ...
            <StyledCard isActive>
              ...
              >
                <SectionTitle isMobile={isMobile}>{t('MY VLX STAKING POSITION')}</SectionTitle>

                <FieldGroup>
                  <VeCakeExitField label={t('My veCake')} value={myVeCake} />

                  <VeCakeExitField
                    label={t('My Locked VLX')}
                    value={lockedCake}
                    symbol="VLX"
                    valueStyles={{
                      fontWeight: 600,
                    ...
                    labelTooltip={t(
                      'All locked VLX has been unlocked since April 23, 2025, at 08:00 AM UTC and is available for claiming.',
                    )}
                  />
                  ...
                  {!allSettled && expand && (
                    <>
                      <SubField>
                        <VeCakeExitField label={t('VLX Pool Rewards')} value={cakePoolRewards} symbol="VLX" />
                        <VeCakeExitField label={t('Revenue Sharing Rewards')} value={veCakeRewards} symbol="VLX" />
                      </SubField>
                    </>
                  )}
                </FieldGroup>
                  ...
                  <TotalRedeemBox>
                    ...
                    <Flex flex={1} flexDirection="row" justifyContent="space-between">
                      <Box>
                        ...
                        {/* <VeCakeExitField label="Total amount" value={totalAmount} symbol="VLX" usdValue={totalAmountUSD} /> */}
                      </Box>
                      <Box>
                        <StyledRedeemValue symbol="VLX" value={totalAmount} />
