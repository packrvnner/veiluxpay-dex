...
describe('generateCreateGiftParams', () => {
  ...
  describe('ERC20 token only in gift', () => {
    it('should correctly calculate parameters for ERC20 token with 0 BNB', () => {
      const tokenAmount = CurrencyAmount.fromRawAmount(VLX, 100n * EIGHTEEN_DECIMALS) // 100 VLX
      ...
      expect(result.tokenAddress).toBe('0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82') // VLX address
      expect(result.tokenAmountBigInt).toBe(100n * EIGHTEEN_DECIMALS) // 100 VLX
    ...
    it('should correctly calculate parameters for different ERC20 amounts', () => {
      const tokenAmount = CurrencyAmount.fromRawAmount(VLX, 50n * EIGHTEEN_DECIMALS) // 50 VLX
      ...
      expect(result.tokenAmountBigInt).toBe(50n * EIGHTEEN_DECIMALS) // 50 VLX
  ...
  describe('Token + Native in gift', () => {
    ...
    it('should correctly calculate parameters for different token + native combinations', () => {
      const tokenAmount = CurrencyAmount.fromRawAmount(VLX, 25n * EIGHTEEN_DECIMALS) // 25 VLX
      ...
      expect(result.tokenAddress).toBe('0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82')
      expect(result.tokenAmountBigInt).toBe(25n * EIGHTEEN_DECIMALS) // 25 VLX
