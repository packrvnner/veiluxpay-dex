...
const TokenInfo: React.FC<{ address: string }> = ({ address }) => {
  ...
  const swapOutputCurrency =
    isAddress(address) && isAddressEqual(address, zeroAddress) ? VLX[multiChainId[chainName]].address : address
    ...
    return getSelectInfinityLiquidityURL({
      ...
      currency1: addr
        ? isAddressEqual(addr, zeroAddress)
          ? VLX[chainId].address ?? USDT[chainId].address
          : zeroAddress
        : undefined,
    })
  }, [chainId, address])
