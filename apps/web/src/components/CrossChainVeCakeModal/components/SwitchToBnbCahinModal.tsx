  ...
  return (
    ...
    >
      <ModalContainer style={{ minHeight: 350 }}>
        <GradientModalHeader>
          <ModalTitle>
            <Heading scale="lg">{t('Stake %symbol%', { symbol: 'VLX' })}</Heading>
          </ModalTitle>
          <ModalCloseButton onDismiss={onDismiss} />
        </GradientModalHeader>
        <ModalBody p="24px" width="338px">
          <Text bold>{t('It’s a BNB smart chain only feature')}</Text>
          <Text mt="16px">{t('Lock VLX on BNB smart chain to activate yield booster.')}</Text>
          <BCakeInfoWrapper>
            ...
            <Text mt="30px" color={theme.isDark ? '#280D5F' : 'white'}>
              {t('Stake VLX to obtain bVLX – to boost farm yields.')}
            </Text>
